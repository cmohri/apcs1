...
public class StatPrinter {
    ...
    { 
	return (i>0 && i<_frequency.size() - 1 && _frequency.get(i-1) < _frequency.get( i )
	...
	for (int x = 1; x + 1 < _frequency.size(); x++){
	    if (isLocalMode(x))
		retArray.add(_frequency.get(x));
	}
	return retArray;
    }
	...
	for (int x = 0; x < _frequency.size(); x++){
	    System.out.print(x + ":");
	    for (int i = 0; i < _frequency.get(x) * (longestBar / max(_frequency)); i++){
		System.out.print("*");
	    }
	    System.out.println();
	} 
